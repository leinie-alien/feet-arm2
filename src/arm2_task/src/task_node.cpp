    ...
    state_(arm2_task::TaskState::IDLE)
  {
    ...
    state_sub_ = this->create_subscription<robot_msgs::msg::RobotState>(
      ...
      [this](const robot_msgs::msg::RobotState::SharedPtr msg) {
        if (!msg || msg->motor_state.size() < 5) { return; }
        for (int i = 0; i < 5; ++i) {
          if (!msg->motor_state[i].valid) { return; }
        }
        std::lock_guard<std::mutex> lock(mtx_);
        if (q_current_.size() != 5) { q_current_ = Eigen::VectorXd::Zero(5); }
        for (int i = 0; i < 5; ++i) {
    ...
    driver_ready_sub_ = this->create_subscription<std_msgs::msg::Bool>(
      ...
      [this](const std_msgs::msg::Bool::SharedPtr msg) {
        if (msg) { driver_ready_.store(msg->data); }
      });
...
private:
  ...
  void load_presets()
  {
    ...
    for (const auto & name : preset_names) {
      ...
      for (int i = 0; i < 5; ++i) { q_rad[i] = angles_deg[i] * M_PI / 180.0; }
      presets_[name] = q_rad;
  ...
  bool send_move_goal(const std::vector<Eigen::VectorXd> & q_waypoints)
  {
    ...
    for (const auto & q : q_waypoints) {
      for (int i = 0; i < 5; ++i) { goal_msg.joint_targets.push_back(q[i]); }
    }
    ...
    send_goal_options.result_callback =
      [this](const GoalHandleMoveJoint::WrappedResult & result) {
        ...
        if (msg.empty()) { msg = succeeded ? "success" : "move_joint action failed."; }
        {
          std::lock_guard<std::mutex> lock(action_result_mutex_);
                               ...
                               geometry_msgs::msg::Pose * out_pose)
  {
    ...
    if (result_future.wait_for(3s) != std::future_status::ready) {
      RCLCPP_ERROR(this->get_logger(), "get_pick_pos service call timed out");
      return false;
    }
  ...
  {
    if (input_cmd == nullptr) { return false; }
  ...
  void do_reset()
  {
    ...
    set_suction(false);
    if (!request_mode_switch("moving")) { return; }
    if (!presets_.count("reset")) {
      RCLCPP_ERROR(this->get_logger(), "Preset 'reset' not found!");
      return;
    }
    if (send_move_goal({presets_["reset"]})) { wait_for_action_completion(); }
  ...
  void do_load()
  {
    ...
    if (send_move_goal({presets_["load"]})) { wait_for_action_completion(); }
  }
  ...
  void do_look_out(const geometry_msgs::msg::Pose & target)
  {
    ...
    goal_q[4]    = 0.0;
    if (send_move_goal({goal_q})) { wait_for_action_completion(); }
  }
  ...
  bool do_grasp_move(const geometry_msgs::msg::Pose & target)
  {
    ...
    const Eigen::Vector3d ee_target(
      target.position.x + tool_offset_x_,
      target.position.y + tool_offset_y_,
    ...
    if (!kin_engine_->solveIK(pre_target, grasp_pitch_, tool_roll, q_pre)) {
      RCLCPP_ERROR(this->get_logger(),
        "[grasp_move] IK failed for pre-grasp (%.3f, %.3f, %.3f)",
        pre_target.x(), pre_target.y(), pre_target.z());
      return false;
    }
    if (!kin_engine_->solveIK(ee_target, grasp_pitch_, tool_roll, q_grasp)) {
      RCLCPP_ERROR(this->get_logger(),
    ...
    RCLCPP_INFO(this->get_logger(),
      ...
      pre_target.z(), ee_target.z(), grasp_pitch_, tool_roll);

    if (!send_move_goal(std::vector<Eigen::VectorXd>{q_pre, q_grasp})) { return false; }
  ...
  bool do_place_move(const geometry_msgs::msg::Pose & target)
  {
    ...
    if (!kin_engine_->solveIK(ee_target, grasp_pitch_, tool_roll, q_place)) {
      RCLCPP_ERROR(this->get_logger(),
    ...
    if (!send_move_goal(std::vector<Eigen::VectorXd>{q_pre, q_place})) { return false; }
    if (!wait_for_action_completion()) { return false; }
    ...
    if (q_current_snap.size() == 5) {
      ...
      if (kin_engine_->solveIK(retreat_pos, grasp_pitch_, tool_roll, q_retreat)) {
        RCLCPP_INFO(this->get_logger(),
          "[place_move] retreating %.2fm upward", place_retreat_offset_);
        if (send_move_goal({q_retreat})) { wait_for_action_completion(); }
      } else {
  ...
  bool do_full_grasp(const geometry_msgs::msg::Pose & target)
  {
    ...
    if (!do_grasp_move(target)) { return false; }
    do_suction_on();
    return true;
  }
  ...
  bool do_full_place(const geometry_msgs::msg::Pose & target)
  {
    ...
    if (!do_place_move(target)) { return false; }
  ...
  bool phase1_get_coarse_target(geometry_msgs::msg::Pose & target_world)
  {
    ...
    fwd.position.x   = 1.0; fwd.position.y = 0.0; fwd.position.z = 0.0;
  ...
  bool phase2_align(geometry_msgs::msg::Pose & target_world)
  {
    ...
    q_overhead[4]    = 0.0;
    if (!send_move_goal({q_overhead})) { return false; }
    if (!wait_for_action_completion()) { return false; }
    wait_joints_still(0.02, 800);
    ...
    for (int iter = 0; iter < align_max_iters_; ++iter) {
      ...
      q_new[4] = 0.0;
      if (!send_move_goal({q_new})) { return false; }
      if (!wait_for_action_completion()) { return false; }
  ...
  bool phase3_grasp_descend(const geometry_msgs::msg::Pose & target_world)
  {
    ...
    if (q_rot.size() == 5) {
      q_rot[4] = -M_PI / 2.0;
      if (!send_move_goal({q_rot})) { return false; }
      if (!wait_for_action_completion()) { return false; }
      wait_joints_still(0.02, 600);
    }
    ...
    if (!do_grasp_move(target_world)) { return false; }
    do_suction_on();
  ...
  bool phase3_place_descend(const geometry_msgs::msg::Pose & target_world)
  {
    ...
    if (!do_place_move(target_world)) { return false; }
  ...
  bool do_3phase_grasp()
  {
    ...
    if (!phase1_get_coarse_target(coarse_target)) {
      RCLCPP_ERROR(get_logger(), "[3PGrasp] Phase1 failed."); return false;
    }

    if (!phase2_align(coarse_target)) {
      RCLCPP_ERROR(get_logger(), "[3PGrasp] Phase2 alignment failed."); return false;
    }

    target_pub_->publish(coarse_target);
    if (!phase3_grasp_descend(coarse_target)) {
      RCLCPP_ERROR(get_logger(), "[3PGrasp] Phase3 grasp failed."); return false;
    }
  ...
  bool do_3phase_place()
  {
    ...
    geometry_msgs::msg::Pose coarse_target;
    if (!phase1_get_coarse_target(coarse_target)) {
      RCLCPP_ERROR(get_logger(), "[3PPlace] Phase1 failed."); return false;
    }

    if (!phase2_align(coarse_target)) {
      RCLCPP_ERROR(get_logger(), "[3PPlace] Phase2 alignment failed."); return false;
    }

    target_pub_->publish(coarse_target);
    if (!phase3_place_descend(coarse_target)) {
      RCLCPP_ERROR(get_logger(), "[3PPlace] Phase3 place failed."); return false;
    }
  ...
  {
    if (!wait_for_system_ready()) { return; }
    ...
    while (rclcpp::ok() && is_running_.load()) {
      ...
      int input_cmd;
      if (!wait_for_user_command(&input_cmd)) { return; }
      ...
      switch (input_cmd) {
        ...
        case 2:
          ...
          q_ << 0, 160, -130, 40, 0;
          q_ *= M_PI / 180.0;
          if (send_move_goal(q_)) { wait_for_action_completion(); }
          break;
