...
Drag-teach recording startup:
1. Launch dm_motor_sdk_ros driver
2. Wait for /robot_driver/ready == true
3. Launch gravity_comp_test_node in teach_drag gains mode
4. Launch teach_drag_record_node to record /arm2/_lowState/joint

Options:
  --arm-params <file>     arm2_task parameter file
  --driver-params <file>  dm_motor_sdk_ros driver parameter file
  --output-file <file>    trajectory CSV output path
...
cleanup() {
    stop_process_group "${RECORD_PID}" "teach_drag_record_node"
...
echo "[INFO] Launching gravity_comp_test_node in teach_drag mode..."
launch_in_group GRAVITY_PID ros2 run arm2_task gravity_comp_test_node --ros-args --params-file "${ARM_PARAMS_FILE}" -p gravity_comp_test.gains_mode:=teach_drag

RECORD_CMD=(ros2 run arm2_task teach_drag_record_node --ros-args --params-file "${ARM_PARAMS_FILE}")
if [[ -n "${OUTPUT_FILE}" ]]; then
    RECORD_CMD+=(-p "teach_drag_record.output_file:=${OUTPUT_FILE}")
fi

echo "[INFO] Launching teach_drag_record_node..."
launch_in_group RECORD_PID "${RECORD_CMD[@]}"
